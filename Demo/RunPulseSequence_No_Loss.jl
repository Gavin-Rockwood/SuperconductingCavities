...
SC.RunPulseSequence(Mode3, ρ, Mode3.Stuff["Drive Sequences"]["Binomial_Encoding"], c_ops = c_ops, run_name = "Run_Loss_"*string(with_loss)*"_"*string(now()), solver_kwargs = solver_kwargs, spps = 0.5)
end_time = now()
